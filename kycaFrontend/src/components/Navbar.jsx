    ...
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            ...
            <nav className="hidden md:flex space-x-10">
              ...
              <Link to="/gem" className="nav-link" onClick={closeMenu}>
                KYCA Gems
              </Link>
              {user.isAdmin && checkTokenOnLoad() && (
                <Link to="/adminpanel" className="nav-link" onClick={closeMenu}>
                  Admin Panel
                </Link>
              )}
            </nav>
      ...
      >
        ...
        <nav className="flex flex-col items-start space-y-4 p-5">
          ...
          <Link to="/testimonial" className="nav-link" onClick={closeMenu}>
            Testimonials
          </Link>
          <Link to="/gem" className="nav-link" onClick={closeMenu}>
            KYCA Gems
        ...
        <div className="p-5">
          {user.name === "" && !checkTokenOnLoad() ? (
            <button
              onClick={handleLogin}
              className="bg-emerald-500 text-white px-4 py-2 w-full rounded-md hover:bg-emerald-600 transition"
            >
              Login
            </button>
          ) : (
